...
const unsigned long TRACK_INTERVAL = 10;  
...
void updateSolarTracker() {
  ...
  float peakADC = max(max(TL, TR), max(BL, BR));
  float ldrVoltage = (peakADC / 1023.0) * 5.0;
  float r_ldr = (47000.0 * (5.0 - ldrVoltage)) / (ldrVoltage + 0.000001);
  peakLDR = pow((50000.0 / r_ldr), (1.0 / 0.85));
